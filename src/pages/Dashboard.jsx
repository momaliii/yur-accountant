...
export default function Dashboard() {
  const { clients, income, expenses, debts, openingBalances, getOpeningBalance } = useDataStore();
  ...
  const stats = useMemo(() => {
    ...
    const openingBalance = getOpeningBalance('month', currentMonthStr);
    const openingBalanceAmount = openingBalance
      ? currencyService.convert(openingBalance.amount, openingBalance.currency || baseCurrency, baseCurrency, exchangeRates)
      : 0;
    
    // Calculate total balance (opening balance + net profit)
    const totalBalance = openingBalanceAmount + netProfit;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Stat Cards */}
      <div className={`grid grid-cols-1 sm:grid-cols-2 ${stats.openingBalance !== 0 ? 'lg:grid-cols-5' : 'lg:grid-cols-4'} gap-4`}>
        <StatCard
        ...
        {stats.openingBalance !== 0 && (
          <StatCard
            title="Total Balance"
            value={<PrivacyValue value={formatCurrency(stats.totalBalance)} />}
            icon={Wallet}
            color={stats.totalBalance >= 0 ? 'emerald' : 'red'}
            trend="up"
            trendValue={`Opening: ${formatCurrency(stats.openingBalance)}`}
          />
        )}
        <StatCard
